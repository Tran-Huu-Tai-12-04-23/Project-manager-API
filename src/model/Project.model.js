...
const forceProject = async (projectId) => {
    try {
        const newProject = await Project.findOneAndDelete({ _id: projectId });
        if (newProject) {
            let columns = newProject.columns;
            const result = await removeColumnFrProject(columns);
            return result;
        }
        return false;
    } catch (error) {
...
const updateProject = async (projectId, newListFileId, newData) => {
    try {
        ...
        if (project) {
            ...
            const result = await Project.findOneAndUpdate({ _id: projectId }, projectUpdate, { new: true });
            return result;
        }
        return false;
    } catch (error) {
