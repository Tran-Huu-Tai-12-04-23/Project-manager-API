...
const { getTaskWithHighestOrder, createNewTask } = require('../model/Task.model');

const { createNewNote, deleteOneNote, findNotesByUserAndProject } = require('../model/Note.model');
...
class ProjectController {
    ...
    async getProjectsDelete(req, res, next) {
        try {
            ...
            const condition = {
                $and: [{ createdBy: userId, is_delete: true }],
            };
            const result = await getProjectsInfo(condition);
    ...
    async removeProject(req, res, next) {
        try {
            ...
            console.error(error);
            next(error); // An error occurred during the deletion process
        }
    }
